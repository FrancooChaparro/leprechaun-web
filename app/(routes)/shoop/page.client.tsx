...
const Shoop = ({products}:{products: Product[]}) => {
...
//   const { Productos } = useMyContext();
  ...
  const fill = () => {
    ...
    } else if (products.length <= showMoreProducts) {
      return `Mostrando ${products.length} productos`;
    } else {
      return `Mostrando 1-${showMoreProducts} de ${products.length} productos`;
    }
  };
  
  const handleShowMore = () => {
    if (showMoreProducts + 20 > products.length) {
      setShowMoreProducts(products.length); 
    } else {
  ...
  return (
    <div className={styles["container-main-shoop"]}>
      ...
      {showMoreProducts < products.length && (
        <div
          style={{
            width: "100%",
            display: "flex",
